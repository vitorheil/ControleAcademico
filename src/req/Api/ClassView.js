...
async function createClass(name, vacancy,dateStart,dateEnd,enrroled,subject){
    ...
    const req = await http.post(path, {name: name, vacancy: vacancy, dateStart:dateStart, dateEnd:dateEnd, enrroled:enrroled, subject: subject}, {headers: await headers.headers()})
    return req
}
...
async function updateClass(id, name, vacancy,dateStart,dateEnd,enrroled,subject){
    ...
    const req = await http.put(path, {name: name, vacancy: vacancy, dateStart:dateStart, dateEnd:dateEnd, enrroled:enrroled, subject: subject}, {headers: await headers.headers()})
    return req
}
